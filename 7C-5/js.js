...
async function peticion(urlServidor) {
    ...
    try {
        let promesa = fetch(urlServidor, options)
...
async function obtenerDatos(url) {
    ...
    let datosJson =await peticion(url);
  ...
  document.getElementById('botAñadir').addEventListener('click', () => {
    obtenerDatos('https://reqres.in/api/users');
  });
  ...
  function generarCabecera(tabla) {
    ...
    const celdasCabecera = '<th>id</th><th>Nombre</th><th>Apellido</th><th>createdAt</th>';
  ...
  function generarFila(usuario) {
    ...
    const celdaEmail = usuario.createdAt;
